...
export const cycleSmoke: SmokeTrait = stage => ({
    id: 'stage3-smoke',
    enabled: true,
    when: stage.when,
    progress: (player) => stage.progress(player),
    style: (_player, x) => stage3SmokeGradient(x),
    perTargetScale: {
        'rear-left': 1.2,
        'rear-right': 1.2,
        'front-left': 0.2,
...
export const darkSmoke: SmokeTrait = stage => ({
    id: stage.id === 'stage5' ? 'stage5-smoke' : 'stage6-smoke',
    enabled: true,
    when: stage.when,
    progress: (player) => stage.progress(player),
    style: (_player, x) => (stage.id === 'stage5' ? stage5SmokeGradient : stage6SmokeGradient)(x),
    perTargetScale: {
        'rear-left': 1.3,
        'rear-right': 1.3,
        'front-left': 0.2,
